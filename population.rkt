...
(define (build-random-population n)
  (build-vector n (lambda (_) (make-random-machine 1 1))))
...
(define (population-reset population)
  (for ([auto population] [i (in-naturals)])
    (vector-set! population i (reset-machine auto))))
...
(define (regenerate population rate)
  ...
  (for ([i (in-range rate)]
    ...
    (vector-set! population i (reset-machine (vector-ref population auto))))
  (shuffle-vector population))

;; MUTATE
(define (mutate-population population rate)
  (for ([i (in-range rate)])
    (vector-set! population i (mutate-machine (vector-ref population i)))))
...
(define (evolve population cycles speed rounds delta mutation rank-file)
  (cond
   [(zero? cycles) '()]
   [else
    (define rankings (scan-flatten (vector-map reset-machine population)))
